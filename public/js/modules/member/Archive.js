...
com.em.MemberFile.prototype = {
	...
	setHeaders: function(pheaders){with(this) {
		...
		$("#check-master").bind('click', function() {
			...
			if (checked) {
				$('#tblMemberFile input[id^=check-slave-]').attr('checked', true);
			} else {
				$('#tblMemberFile input[id^=check-slave-]').attr('checked', false);
			}
		});		
	}},
	...
	configureTable: function(selector, pdestroy) { with (this) {
		table = $(selector).dataTable({
			...
			"fnDrawCallback": function() {
				clickToUpdate('#tblMemberFile a[id^=update-memberFile-]');
			},
	...
	clickToDelete: function(selector) {with (this) {
		$(selector).bind('click',function(event) {
			...
			var items = $('#tblMemberFile :checked');
			...
			jConfirm('Are you sure to delete?', 'Delete Archive', function(r) {			    
			    if (r) {
			    	$.ajax({
						dataType: 'json', 
						type: "POST", 
