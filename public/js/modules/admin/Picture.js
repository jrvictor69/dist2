...
com.em.Picture.prototype = {
	...
	setHeaders: function(pheaders){with(this) {
		...
		$("#check-master").bind('click', function() {
			...
			if (checked) {
				$('#tblPicture input[id^=check-slave-]').attr('checked', true);
			} else {
				$('#tblPicture input[id^=check-slave-]').attr('checked', false);
			}
		});		
	}},
	...
	clickToDelete: function(selector) {with (this) {
		$(selector).bind('click',function(event) {
			...
			jConfirm('Are you sure to delete?', 'Delete Picture', function(r) {			    
			    if (r) {
			    	$.ajax({
						dataType: 'json', 
						type: "POST", 
