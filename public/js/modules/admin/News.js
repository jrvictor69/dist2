...
//		oSettings._iDisplayLength = 3;
	...
	setHeaders: function(pheaders){with(this) {
		...
		$("#check-master").bind('click', function() {
			...
			if (checked) {
				$('#tblNews input[id^=check-slave-]').attr('checked', true);
			} else {
				$('#tblNews input[id^=check-slave-]').attr('checked', false);
			}
		});		
	}},
...
//		});
	...
	clickToDelete: function(selector) {with (this) {
		$(selector).bind('click',function(event) {
			...
			jConfirm('Are you sure to delete?', 'Delete News', function(r) {			    
			    if (r) {
			    	$.ajax({
						dataType: 'json', 
						type: "POST", 
