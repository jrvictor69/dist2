...
com.em.Privilege.prototype = {
	...
	setHeaders: function(pheaders){with(this) {
		...
		$("#check-master").bind('click', function() {
			...
			if (checked) {
				$('#tblPrivilege input[id^=check-slave-]').attr('checked', true);
			} else {
				$('#tblPrivilege input[id^=check-slave-]').attr('checked', false);
			}
		});		
	}},
	...
	configureTable: function(selector, pdestroy) { with (this) {
		table = $(selector).dataTable({
			...
			"fnDrawCallback": function() {
				clickToUpdate('#tblPrivilege a[id^=update-privilege-]');
			},
	...
	clickToDelete: function(selector) {with (this) {
		$(selector).bind('click',function(event) {
			...
			jConfirm('Are you sure to delete?', 'Delete Privilege', function(r) {			    
			    if (r) {
			    	$.ajax({
						dataType: 'json', 
						type: "POST", 
