...
com.em.UnityClub.prototype = {
	...
	setHeaders: function(pheaders){with(this) {
		...
		$("#check-master").bind('click', function() {
			...
			if (checked) {
				$('#tblUnityClub input[id^=check-slave-]').attr('checked', true);
			} else {
				$('#tblUnityClub input[id^=check-slave-]').attr('checked', false);
			}
		});
	}},
	...
	configureTable: function(selector, pdestroy) { with (this) {
		table = $(selector).dataTable({
			...
			"fnDrawCallback": function() {
				clickToUpdate('#tblUnityClub a[id^=update-unityClub-]');
			},
	...
	getColumns: function() {with (this) {
		...
		columns.push({
			...
			fnRender : function (oObj){
				return '<a id="update-unityClub-'+oObj.aData[0]+'" href="'+url.toUpdate+'/id/'+oObj.aData[0]+'">'+oObj.aData[1]+'</a>';
				}
			});
		columns.push({"sWidth": "20%"});
		columns.push({"sWidth": "23%"});
	...
	clickToDelete: function(selector) {with (this) {
		$(selector).bind('click',function(event) {
			...
			var items = $('#tblUnityClub :checked');
			...
			jConfirm('Are you sure to delete?', 'Delete UnityClub', function(r) {			    
				if (r) {
					$.ajax({
						dataType: 'json', 
						type: "POST", 
