...
com.em.UserGroup.prototype = {
	...
	setHeaders: function(pheaders){with(this) {
		...
		$("#check-master").bind('click', function() {
			...
			if (checked) {
				$('#tblUserGroup input[id^=check-slave-]').attr('checked', true);
			} else {
				$('#tblUserGroup input[id^=check-slave-]').attr('checked', false);
			}
		});		
	}},
	...
	configureTable: function(selector, pdestroy) { with (this) {
		table = $(selector).dataTable({
			...
			"oLanguage": {
				"sEmptyTable": "No User Group found."
			},
			"fnDrawCallback": function() {
				clickToUpdate('#tblUserGroup a[id^=update-userGroup-]');
			},
	...
	getColumns: function() {with (this) {
		...
		columns.push({
			...
			fnRender : function (oObj){
				return '<a id="update-userGroup-'+oObj.aData[0]+'" href="'+url.toUpdate+'/id/'+oObj.aData[0]+'">'+oObj.aData[1]+'</a>';
				}
			});
	...
	clickToDelete: function(selector) {with (this) {
		$(selector).bind('click',function(event) {
			...
			var items = $('#tblUserGroup :checked');
			...
			jConfirm('Are you sure to delete?', 'Delete User Group', function(r) {			    
			    if (r) {
			    	$.ajax({
						dataType: 'json', 
						type: "POST", 
