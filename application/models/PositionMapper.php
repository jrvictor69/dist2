...
class Model_PositionMapper extends Model_TemporalMapper {
    ...
    public function getDbTable() {
        if (null === $this->_dbTable) {
            $this->setDbTable('Model_DbTable_Position');
        }
     ...
     * 
     * Saves model
     * @param Model_Position $position
     */
	public function save(Model_Position $position) {
		$data = array(
     ...
     * @param Model_Position $position
     */
	public function update($id, Model_Position $position) {
        ...
        if (0 == count($result)) {
	 	...
	 	$data = array(
            ...
            'changed' => date('Y-m-d H:i:s'),
	 		'changedBy' => $position->getChangedBy()
        );
    ...
    /**
     * 
     * Verifies if the name position already exist it.
     * @param string $name
     * @return boolean
     */
	...
	/**
     * 
     * Verifies if the id and name of the position already exist.
     * @param int $id
     * @param string $name
     * @return boolean
     */
